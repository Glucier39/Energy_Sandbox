...
nn_model <- neuralnet(
  FutureLMP_Outcome ~ total_lmp_da + day_of_year + mw + tmpf + dwpf + relh + wind_generation_mw + solar_generation_mw,
  data = train_NN_DF,
  hidden = 5,
  act.fct = "logistic",
  linear.output = FALSE,
  stepmax = 2e6, # accuracy controls
  threshold = 0.001,
  lifesign = "full" # details 
)
...
nn_confusion <- confusionMatrix(Up_Predicted, Up_Outcome)


#saveRDS(nn_model, file = "nn_model.rds")
#nn_model <- readRDS(file = "nn_model.rds")
plot(nn_model)
nn_confusion
```
...
```{r}
nn_table <- as.table(nn_confusion$table)
cm_melted <- melt(cm_table)
colnames(cm_melted) <- c("Reference", "Prediction", "Count")

ggplot(data = cm_melted, aes(x = Reference, y = Prediction, fill = Count)) +
  geom_tile(color = "white") +
  geom_text(aes(label = Count), color = "black", size = 5) +
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  labs(title = "Confusion Matrix", x = "Actual Class", y = "Predicted Class") +
  theme_minimal(base_size = 15) + labs(x = "True Class (Actual)", y = "Predicted Class", fill = "Count")
